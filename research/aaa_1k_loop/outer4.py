"""Iteration 0004 challenger stages: develop, attack, freeze, confirm.

    python -m research.aaa_1k_loop.outer4 develop
    python -m research.aaa_1k_loop.outer4 attack
    python -m research.aaa_1k_loop.outer4 freeze      # then commit the freeze
    python -m research.aaa_1k_loop.outer4 confirm     # observes the confirmation once

This module is on the confirmation path, so everything it imports is in
:data:`CONFIRMATION_SOURCES_4` and a test holds it to that. It deliberately
...
CONFIRMATION_SOURCES_4 = (
    ...
    "research/aaa_1k_loop/outer4.py",
    "research/aaa_1k_loop/unfolding.py",
    "aaa/noise/reservation.py",
)
"""Iteration 0004's confirmation path, in addition to every AAA-1K phase file."""
...
def command_develop(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema": "aaa.loop.development.v4",
...
def command_freeze(_args: argparse.Namespace) -> int:
    ...
    ledger = reserve(
        ...
        purpose="iteration 0004 fresh confirmation streams (M2 challenger)",
        iteration_id=ITERATION_ID,
    )
    ledger = reserve(
        ledger,
        ...
        purpose="iteration 0004 fresh confirmation initializations (M2 challenger)",
        iteration_id=ITERATION_ID,
    )
    ...
    manifest = build_freeze(
        ...
        sources=CONFIRMATION_SOURCES_4,
    )
...
def command_confirm(args: argparse.Namespace) -> int:
    ...
    problems = verify_freeze(
        manifest, root, ledger=ledger, frozen_content=frozen_content(selected), sources=CONFIRMATION_SOURCES_4
    )
    for block_id in (CONFIRMATION_ENV_BLOCK, CONFIRMATION_INIT_BLOCK):
        try:
    ...
    try:
        payload["decision"] = decide4(primitives, selected)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m research.aaa_1k_loop.outer4")
    sub = parser.add_subparsers(dest="command", required=True)
    develop = sub.add_parser("develop", help="screen both precommitted candidates on the development block")
