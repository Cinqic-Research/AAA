...
confirmation is the first real use of the outer half; generalize after it.
    ...
    python -m research.aaa_1k_loop.outer6 freeze      # then commit the freeze
    python -m research.aaa_1k_loop.outer6 confirm     # observes the confirmation once

This module is on the confirmation path, so everything it imports is in
:data:`CONFIRMATION_SOURCES_6` and a test holds it to that. It deliberately
...
CONFIRMATION_SOURCES_6 = (
    ...
    "research/aaa_1k_loop/outer6.py",
    "research/aaa_1k_loop/unfolding.py",
    "aaa/noise/reservation.py",
)
"""Iteration 0006's confirmation path, in addition to every AAA-1K phase file."""
...
def command_develop(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema": "aaa.loop.development.v6",
...
def command_freeze(_args: argparse.Namespace) -> int:
    ...
    manifest = build_freeze(
        ...
        sources=CONFIRMATION_SOURCES_6,
    )
...
def command_confirm(args: argparse.Namespace) -> int:
    ...
    problems = verify_freeze(
        manifest, root, ledger=ledger, frozen_content=frozen_content(selected), sources=CONFIRMATION_SOURCES_6
    )
    for block_id in (CONFIRMATION_ENV_BLOCK, CONFIRMATION_INIT_BLOCK):
        try:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m research.aaa_1k_loop.outer6")
