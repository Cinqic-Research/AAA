...
fingerprint (`5ce6e019…`), Champion 0, Champion 1 and every historical artifact
are unchanged and still verify.

- **Compute.** `aaa/compute/` adds an explicit device grammar
  (`cpu|cuda|cuda:N|auto`, no silent fallback), backend provenance and a
  privacy-preserving hardware probe. The batched v2 learner runs on NumPy or
  CuPy through one code path; `requirements-cuda-lock.txt` pins CuPy 14.2 and
  the NVIDIA CUDA 13 runtime wheels. Qualification on FLOWBOX
  (`docs/aaa_1k_v2_compute_report.md`): one CPU core for single streams, the
  8-core CPU up to about 2,000 lockstep cells, and the RTX 2060 only from about
  4,096 cells. Sweeps and external benchmarks stay on the CPU.
- **Phase.** 26 internal stress families (4 held out), NARMA-10/20,
  Mackey-Glass, 12 dysts systems and 4 Monash datasets. River was assessed and
  not adopted; Gymnasium and MLPerf Tiny were rejected. Identities are 64-bit
  with a fail-closed disjointness proof (resolving `AAA-163` for new work).
  Every adaptive-state scalar is audited.
- **Result: no change.** Development found no challenger (V2-D15). The
  characterization-only confirmation (`NO_CHALLENGER`, recomputed) shows every
  candidate behind Champion 1 on stress and held-out families. Capacity
  verdict: `NOT_CAPACITY_LIMITED`. Diagnostics: M1 is a coarse-observation
  operating-point effect; `AAA-162`'s original characterization is refuted;
  `AAA-172` shows no lock or stall; stateful optimizers do not earn their
  state (V2-D16).
- **Findings that revise earlier statements.** Round 3's Q2 adaptation result
  does not replicate on fresh identities. Retention resolves in the good
  direction. The error-head loss is inert for accuracy.
- **New issues.** `AAA-179` (provenance captured after a run; repaired before
  any evidence) and `AAA-180` (a latent K5 defect with no effect in this
  phase).
- Report: `docs/aaa_1k_v2_report.md`, generated by
  `tools/write_aaa_1k_v2_report.py` and guarded by
...
- Dependabot no longer opens `pip` pull requests. `requirements-lock.txt` is
  ...
  (`AAA-175`).
  The policy was confirmed on 2026-09-23. Alerts remain the notification
  path, and a lock change stays a deliberate, recorded decision.
