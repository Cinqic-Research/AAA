...
## The evidence-gated pre-scale phase (2026-09-24)

| Where | Statement | Now |
|---|---|---|
| `aaa_1k_v2_report.md` ("The error head"), generated by `tools/write_aaa_1k_v2_report.py` | the error head "under-predicts" the scale of realized error | **wrong direction** (`AAA-201`): its bias `mean(estimate - realized)` is positive in 2,771 of 3,328 Champion 1 cells; it over-predicts on average, and a calibration slope of 0.38 means its estimates vary more than realized error tracks them. The conclusion that it is a weak uncertainty signal stands |
| `aaa_python_development_report.md`, `aaa_python_research_brief.md`, `scaling_readiness.md` | the v0 learner "loses clearly to surface heuristics on syntax" | true for v0's declared two-epoch budget; the learner class was under-trained (`AAA-197`): more passes over the same data raise syntax from 0.53 to 0.89 |
