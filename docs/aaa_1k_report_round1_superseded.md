> **Superseded.** This is the round-1 report, retained unchanged as historical evidence.
> Four design defects in it were found by probing its own conclusions and repaired in
> round 2; see [`aaa_1k_report.md`](aaa_1k_report.md) for the current result and
> [`aaa_1k_self_review.md`](aaa_1k_self_review.md) for what was wrong. In particular its
> Q2 does not measure adaptation, its Q5 does not measure retention, its intervals are
> too narrow, and its neural controls were run on hyperparameters selected for a
